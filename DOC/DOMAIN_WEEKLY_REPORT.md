...
- **역할:** 프론트엔드로부터 주간 보고 날짜와 4개의 엑셀 파일을 `UploadFile`로 수신. 완료된 최종 포맷팅 텍스트(String)를 프론트엔드로 반환.
...
## 4. 프론트엔드 설계
- **UI:** 1. 날짜 선택(DatePicker) 및 다중 파일 업로드 영역.
  2. **결과 출력 영역:** 백엔드에서 응답받은 최종 텍스트를 보여주는 넓은 텍스트 영역(`textarea` 또는 `div`).
  3. **복사 버튼:** 클릭 시 결과 텍스트를 클립보드에 복사하는 버튼(Copy to Clipboard) 필수 구현.
- **통신:** Zustand 스토어 활용, FastAPI 백엔드로 전송 및 응답 수신.
